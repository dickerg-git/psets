...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        #Initialize the contents of each node in the Network graph
        for a_node in nodes:
            a_node.parent = None
            a_node.node_key = None

        print("Searching from ", source, destination)
        print("Distance is :", weight(source,destination))

        # Insert the starting vertex into the min heap.
        minQ.insert(NodeDistancePair(source,0))
        #next_node = source

        # Start the search algorithm loop
        while len(minQ) > 0:

            #Advance to the node closest to the source node
            next_node_pair = minQ.extract_min()
            next_node = next_node_pair.node
            print("Trying ", next_node)

            #Did we make it to the destination node???
            if( next_node is destination): break

            #Find the list of nodes adjacent to the current node
            #print(source.adj)
            for new_node in next_node.adj:
                new_dist = weight(next_node, new_node) # New node delta from the source node
                #print("Adj node: ", new_node)
                #print("Adj dist: ", new_dist)
                #To relax each edge, insert its distance into the min heap
                if( new_node.node_key is None ):
                   # To relax each un-visited node, insert its distance into the min heap
                   new_node.node_key = NodeDistancePair(new_node, (new_dist + next_node_pair.distance) )
                   minQ.insert( new_node.node_key )
                   new_node.parent = next_node
                   nodes_visited += 1
                elif( new_node.node_key.distance > (new_dist + next_node_pair.distance) ):
                    minQ.decrease_key(new_node.node_key)


        #while len(minQ) > 0:
            #    next_node_pair = minQ.extract_min()
            #    print("Next node: ", next_node_pair.node)
            #    print("Min Length:", next_node_pair.distance)



        print("Dijkstra function done!", nodes_visited)
        print("Arrived at : ", next_node)
        print("Path length is ", next_node.node_key.distance)
        shortest_path_list.append(next_node)
        shortest_path_list.append(source)

        #while (next_node.parent != None):
            #shortest_path_list.append(next_node)
            #next_node = next_node.parent

        print("Shortest path nodes: ", len(shortest_path_list))
        shortest_path_list.reverse()


        return shortest_path_list, nodes_visited
